...
bool kangaroo::start( )
{
	...
	if( !power_down_sub )
		power_down_sub = nh.subscribe( "power_down", 1, &kangaroo::PowerDownCB, this );
	if( !joint_state_pub)
...
void kangaroo::JointTrajCB(const trajectory_msgs::JointTrajectoryPtr &msg)
{
	...
	{
		ROS_INFO("Got a JointTrajectory message whose points have no velocities");
		return;
	}
...
bool kangaroo::send_start_signals(unsigned char address)
{
	...
	int num_of_bytes_1 = write_kangaroo_start_command(address, 'D', buffer);
    ROS_INFO("Sending start signals");
	if (0 > write(fd, buffer, num_of_bytes_1))
	{
		ROS_ERROR("Failed to start channel 1: %s", strerror(errno));
		close();
		return false;
	}

	// send the start signal to channel 2
	int num_of_bytes_2 = write_kangaroo_start_command(address, 'T', buffer);

	if (0 > write(fd, buffer, num_of_bytes_2))
	{
		ROS_ERROR("Failed to start channel 2: %s", strerror(errno));
		close();
		return false;
	}
...
bool kangaroo::send_power_down_signals(unsigned char address)
{
	...
	int num_of_bytes_1 = power_down_all_command(address, 'D', buffer);
    ROS_INFO("Drive channel powered down: %i", address);
	if (0 > write(fd, buffer, num_of_bytes_1))
	{
		ROS_ERROR("Failed to power down channel 1: %s", strerror(errno));
		close();
		return false;
	}

	// send the power_down signal to turn channel 
	int num_of_bytes_2 = power_down_all_command(address, 'T', buffer);
    ROS_INFO("Turn channel powered down: %i", address);
	if (0 > write(fd, buffer, num_of_bytes_2))
	{
		ROS_ERROR("Failed to power down channel 2: %s", strerror(errno));
		close();
		return false;
	}
...
{
    if(channel == 0) ROS_INFO("Channel is null");				
	//ROS_INFO("Sending %f to address %u channel %c ...", speed, address, channel);
	if (!is_open() && !open())
		return false;
...
void kangaroo::JointStateCB( const ros::WallTimerEvent &e )
{
	try
	{
		...
		msg->position[0] = get_parameter((unsigned char)128, 'D', (unsigned char)1);	// position for ch1
		msg->velocity[0] = get_parameter((unsigned char)128, 'D', (unsigned char)2);	// velocity for ch1
		msg->position[1] = get_parameter((unsigned char)128, 'T', (unsigned char)1);	// position for ch2
		msg->velocity[1] = get_parameter((unsigned char)128, 'T', (unsigned char)2);	// velocity for ch2
